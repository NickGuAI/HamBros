...
describe('CommanderAgent system prompt injection', () => {
  ...
  it('injects memory context for task pickup and heartbeat', async () => {
    ...
    expect(taskPickup.systemPrompt).toContain('## Task Management')
    expect(taskPickup.systemPrompt).toContain('gh issue list --label commander')
    ...
    expect(heartbeat.systemPrompt).toContain('## Task Management')
